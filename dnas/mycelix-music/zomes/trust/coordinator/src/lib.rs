...
pub fn get_trust_claims(agent: AgentPubKey) -> ExternResult<Vec<TrustClaim>> {
    ...
    let links = get_links(
        GetLinksInputBuilder::try_new(to_path.path_entry_hash()?, LinkTypes::AgentToClaimsReceived)?
            .build(),
    )?;
...
pub fn get_all_cdn_nodes(_: ()) -> ExternResult<Vec<CdnNodeReputation>> {
    let all_nodes_path = Path::from("all_cdn_nodes");
    let links = get_links(
        GetLinksInputBuilder::try_new(all_nodes_path.path_entry_hash()?, LinkTypes::NodeToReputation)?
            .build(),
    )?;
...
pub fn get_my_trust_claims(_: ()) -> ExternResult<Vec<TrustClaim>> {
    ...
    let links = get_links(
        GetLinksInputBuilder::try_new(from_path.path_entry_hash()?, LinkTypes::AgentToClaimsMade)?
            .build(),
    )?;
